...
-- Migration 005: Complete Social Features Setup
-- Creates all tables with correct name 'trips' (not 'public_trips')
...
CREATE INDEX IF NOT EXISTS idx_trips_popular ON public.trips (likes_count DESC, views_count DESC)
    WHERE is_hidden = FALSE;

CREATE INDEX IF NOT EXISTS idx_trips_recent ON public.trips (created_at DESC)
    WHERE is_hidden = FALSE;

CREATE INDEX IF NOT EXISTS idx_trips_featured ON public.trips (created_at DESC)
...
CREATE INDEX IF NOT EXISTS idx_trips_region ON public.trips (country_code, region)
    WHERE is_hidden = FALSE;
...
CREATE INDEX IF NOT EXISTS idx_user_profiles_name ON public.user_profiles (display_name);
...
-- Trip Likes Policies
DROP POLICY IF EXISTS "Likes are publicly readable" ON public.trip_likes;
CREATE POLICY "Likes are publicly readable" ON public.trip_likes
    FOR SELECT USING (TRUE);

DROP POLICY IF EXISTS "Users can insert own likes" ON public.trip_likes;
CREATE POLICY "Users can insert own likes" ON public.trip_likes
    FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own likes" ON public.trip_likes;
CREATE POLICY "Users can delete own likes" ON public.trip_likes
    FOR DELETE USING (auth.uid() = user_id);

-- Trip Imports Policies
DROP POLICY IF EXISTS "Imports are readable by owner and trip author" ON public.trip_imports;
...
DROP POLICY IF EXISTS "Users can insert own imports" ON public.trip_imports;
