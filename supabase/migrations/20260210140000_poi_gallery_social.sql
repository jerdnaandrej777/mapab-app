...
-- Migration: POI Gallery + Social (poi_posts, Likes, Votes, Reputation)
-- Idempotent: kann beliebig oft ausgefuehrt werden
-- Voraussetzung: 006 (user_profiles), 007 (poi_stats)
...
BEGIN
    ...
    INSERT INTO public.poi_posts (
        poi_id, user_id, title, content, categories, is_must_see, cover_photo_path
    )
    VALUES (
        BTRIM(p_poi_id), v_user, BTRIM(p_title),
...
BEGIN
    ...
    FROM public.poi_votes WHERE post_id = p_post_id;
    RETURN v_score;
END;
$$;
